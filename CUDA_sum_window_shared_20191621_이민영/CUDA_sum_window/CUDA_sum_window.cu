...
__global__ void Sum_n_elements_Kernel_shared(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
	...
	if(thread_id == BLOCK_SIZE - 1) {
		for (i = 0;i < Nf;i++) {
			if (id + i + 1 >= N)shared_buffer[BLOCK_SIZE + i + Nf] = 0;
				...
				shared_buffer[thread_id + i + Nf + 1] = d_ArrayElements[id + i + 1];
			}
		}
	}
	...
	//for (int i = 0;i < Nf+thread_id;i++) {
		
	//	printf("%d %d\n", i, shared_buffer[i]);
	//}
	//printf("%d %d\n", shared_buffer, d_ArrayElements);
	__syncthreads();
