...
void Mat_Vec_Multiply_GPU(float *p_VecX, float *p_MatA, float *p_VecY_G) {
	...
	float* d_X;
	float *d_Y;
	size_t size_A,size_X,size_Y;
	size_t A_wid, A_hei;
	size_t X_wid, X_hei;
	size_t Y_wid, Y_hei;
	
	A_wid = ELEM_PER_VECTOR;
	A_hei = ELEM_PER_VECTOR;
	size_A = A_wid * A_hei*sizeof(float);
	CUDA_CALL(cudaMalloc(&d_A, size_A))
	CUDA_CALL(cudaMemcpy(d_A, p_MatA, size_A, cudaMemcpyHostToDevice))


	X_wid = n;
	X_hei = ELEM_PER_VECTOR;
	size_X = X_wid * X_hei*sizeof(float);
	
	CUDA_CALL(cudaMalloc(&d_X, size_X))
	CUDA_CALL(cudaMemcpy(d_X, p_VecX, size_X, cudaMemcpyHostToDevice))


	Y_wid = n;
	Y_hei = ELEM_PER_VECTOR;
	size_Y = Y_wid * Y_hei*sizeof(float);
	
	CUDA_CALL(cudaMalloc(&d_Y, size_Y))
	...
	CHECK_TIME_INIT_GPU();
	CHECK_TIME_START_GPU();
	Mat_Vec_Multiply_Kernel << < dimGrid, dimBlock >> > (d_Y, d_X, d_A,A_wid);
	CHECK_TIME_END_GPU(device_time);
//	CHECK_TIME_DEST_GPU();
	...
	// any errors encountered during the launch.
	CUDA_CALL(cudaDeviceSynchronize())
	CUDA_CALL(cudaMemcpy(p_VecY_G, d_Y, size_Y, cudaMemcpyDeviceToHost))
