...
void CFilteringView::GaussianFilteringCPU() {
	...
	float* kk = new float[25];

	kk[0] = kk[4] = kk[20] = kk[24] = _1;
	kk[1] = kk[3] = kk[5] = kk[9] = _4;
	kk[15] = kk[19] = kk[21] = kk[23] = _4;
	kk[2] = kk[10] = kk[14] = kk[22] = _6;
	kk[6] = kk[8] = kk[16] = kk[18] = _16;
	kk[7] = kk[11] = kk[13] = kk[17] = _24;
	kk[12] = _36;
		...
		for (c = 0; c < m_width; c++) {
			...
			for (int i = -w; i <= w; i++)
			{
				for (int j = -w; j <= w; j++)
				{
					...
					{
						m_imageBitsFiltered[r * m_width + c] += kk[(i + w) * 5 + j + w] * m_imageBits[(r + i) * m_width + (c + j)];
